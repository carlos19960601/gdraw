...
func (t *Triangle) Mutate() {
	...
	for {
		switch rnd.Intn(3) {
		case 0:
			t.X1 = numutil.ClampInt(t.X1+int(rnd.NormFloat64()*16), -m, w-1+m)
			t.Y1 = numutil.ClampInt(t.Y1+int(rnd.NormFloat64()*16), -m, h-1+m)
		case 1:
			t.X2 = numutil.ClampInt(t.X2+int(rnd.NormFloat64()*16), -m, w-1+m)
			t.Y2 = numutil.ClampInt(t.Y2+int(rnd.NormFloat64()*16), -m, h-1+m)
		case 2:
			t.X3 = numutil.ClampInt(t.X3+int(rnd.NormFloat64()*16), -m, w-1+m)
			t.Y3 = numutil.ClampInt(t.Y3+int(rnd.NormFloat64()*16), -m, h-1+m)
		}
		if t.Valid() {
			break
		}
...
func (t *Triangle) Valid() bool {
	...
	var a1, a2, a3 float64
	{
		...
		x2 /= d2
		y2 /= d2
		a1 = numutil.Degrees(math.Acos(x1*x2 + y1*y2))
	}
	{
		...
		x2 /= d2
		y2 /= d2
		a2 = numutil.Degrees(math.Acos(x1*x2 + y1*y2))
	}
	a3 = 180 - a1 - a2
