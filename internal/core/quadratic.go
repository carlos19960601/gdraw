...
func (q *Quadratic) Mutate() {
	...
	for {
		switch rnd.Intn(3) {
		case 0:
			q.X1 = numutil.Clamp(q.X1+rnd.NormFloat64()*16, -m, float64(w-1+m))
			q.Y1 = numutil.Clamp(q.Y1+rnd.NormFloat64()*16, -m, float64(h-1+m))
		case 1:
			q.X2 = numutil.Clamp(q.X2+rnd.NormFloat64()*16, -m, float64(w-1+m))
			q.Y2 = numutil.Clamp(q.Y2+rnd.NormFloat64()*16, -m, float64(h-1+m))
		case 2:
			q.X3 = numutil.Clamp(q.X3+rnd.NormFloat64()*16, -m, float64(w-1+m))
			q.Y3 = numutil.Clamp(q.Y3+rnd.NormFloat64()*16, -m, float64(h-1+m))
		case 3:
			q.Width = numutil.Clamp(q.Width+rnd.NormFloat64(), 1, 16)
		}
		if q.Valid() {
			break
		}
